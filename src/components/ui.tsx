...
export function Screen({ children, scroll = true }: PropsWithChildren<{ scroll?: boolean }>) {
  ...
  const content = scroll ? (
    <ScrollView contentContainerStyle={styles.screenContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>{children}</ScrollView>
  ) : <View style={styles.screenContent}>{children}</View>;
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <Animated.View style={[styles.flex, { opacity, transform: [{ translateY }] }]}>{content}</Animated.View>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, safe: { flex: 1, backgroundColor: colors.background }, screenContent: { padding: spacing.md, paddingBottom: 132, gap: spacing.md, flexGrow: 1 },
  ...
  eyebrow: { color: colors.primary, fontWeight: '900', letterSpacing: 1, fontSize: 11, textTransform: 'uppercase' }, title: { color: colors.text, fontSize: 30, lineHeight: 36, fontWeight: '900', letterSpacing: -0.5 }, subtitle: { color: colors.muted, fontSize: 15, lineHeight: 21 },
  ...
  chip: { alignSelf: 'flex-start', borderRadius: radii.pill, paddingHorizontal: 10, paddingVertical: 5, backgroundColor: colors.background }, chip_success: { backgroundColor: colors.successSoft }, chip_warning: { backgroundColor: colors.warningSoft }, chip_danger: { backgroundColor: colors.dangerSoft }, chip_neutral: { backgroundColor: colors.backgroundTint },
  chipText: { fontWeight: '800', fontSize: 12 }, chipText_success: { color: colors.success }, chipText_warning: { color: colors.warning }, chipText_danger: { color: colors.danger }, chipText_neutral: { color: colors.muted },
  ...
  sheet: { backgroundColor: colors.surface, borderTopLeftRadius: radii.xl, borderTopRightRadius: radii.xl, padding: spacing.md, paddingBottom: spacing.xl, gap: spacing.md, maxHeight: '88%', ...shadows.floating }, sheetHandle: { width: 42, height: 5, borderRadius: radii.pill, backgroundColor: colors.borderStrong, alignSelf: 'center' },
